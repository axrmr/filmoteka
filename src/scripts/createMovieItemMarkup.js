...
const genresFromStorage = $localStorage.get(GENRES_STORAGE_KEY);

const createMovieItemMarkup = (dataArr, ...genresArr) => {
  const genresToFilter = genresFromStorage ? genresFromStorage : genresArr;
  ...
  return dataArr
    .map(movie => {
      ...
      const releaseDate = movie.release_date
        ...
        : '';
      const genres = filterGenres(genresToFilter, movie.genre_ids)
        .slice(0, 2)
        .join(', ');
